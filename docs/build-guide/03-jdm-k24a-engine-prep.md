---
layout: page
title: "03 — JDM K24A/RBB engine preparation"
---
...
1. clear photo of the **RBB cylinder-head casting**
2. photo under the valve cover showing the expected performance VTEC valvetrain / cam arrangement
...
## 3.2 JDM accessory differences that matter

KPower specifically warns that JDM RBB engines differ from US-market K24A2s in the accessory drive. The JDM:
...
KPower's recommended conversion is:
...
For a North-American build, doing this on the stand is a high-value reliability choice because replacement parts are easier to source later.
...
This part directly affects how much firewall work is required. Do not finalize firewall shape using the stock rear-facing outlet if you do not intend to run it.

## 3.6 Oil pump decision

### Reference street / track-day choice
For the KPower tubular-subframe architecture, KPower explicitly recognizes a Unit2 baffled steel pan made for the **K24 RBB balance-shaft oil pump**, so a K20 pump conversion is not automatically required.
...
### Upgrade choice
A K20/PRB-style pump conversion can be appropriate for a sustained-high-rpm build, but it adds cost and changes the required pickup/pan/baffle combination.

Do not buy the oil pan before the pump decision is frozen.
...
- confirm piston-to-valve and cam/tune constraints for the exact engine
- set ECU VTC limits
- do not assume a generic basemap's limits are safe for an unknown JDM engine

## Engine Gate G1

Pass only when:

- RBB engine identity is credible
